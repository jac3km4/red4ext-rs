...
fn dump_player_info(player: Ref<PlayerPuppet>) {
    ...
    let class_name = Ref::<PlayerPuppet>::get_class_name(player.clone());
    info!("class name: {}", class_name);
    let is_ent_entity = Ref::<PlayerPuppet>::is_a(player.clone(), CName::new("entEntity"));
    info!("is an entEntity: {}", is_ent_entity);
    let is_exactly_a_player_puppet =
        Ref::<PlayerPuppet>::is_exactly_a(player.clone(), CName::new(PlayerPuppet::NAME));
