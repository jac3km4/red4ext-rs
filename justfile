...
example_dir        := join(justfile_directory(), "example")
redscript_repo_dir := join(example_dir, "reds")
...
install target='release':
 ...
 cd '{{example_dir}}'; \
 if ('{{target}}' -EQ 'debug') { cargo +nightly build; } else { cargo +nightly build --release; } \
